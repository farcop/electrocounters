...
# miliWATT !!
WATT_HOUR_CORRECTION_T2=5923
WATT_HOUR_CORRECTION_T3=5000
  ...
  case $command in
    ...
    '08') {
            case $command2 in
             ...
             '16') answer="`form_com x${CSN}x00x8Cx13`" ;;
...
CorrectionCounter()
{
  ...
  echo "$1" | awk -Fx -v cor=$correct_watt -v cors=${3} 'function preob(val)
                 {
