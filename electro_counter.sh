...
trap "trap_x 2" 2
 ...
 [ $DEBUG -gt 0 ] && echo "OS: ${OS}; DEVICE: ${DEVICE}; COUNTER_SN: ${COUNTER_SN}($CSN); COUNTER_TYPE: ${COUNTER_TYPE}; TEST_COMMUNITY: ${TEST_COMMUNITY}; DEV_EMULATOR: ${DEV_EMULATOR}; METHOD_COMMUNITY: ${METHOD_COMMUNITY}; FUZZY_MATCHING_CRC: ${FUZZY_MATCHING_CRC}"
 ...
 # COMMAND TEST
 [ ${TEST_COMMUNITY} = "YES" ] && {
     CheckParameter "command" "test" "${COMMANDS}" "" "Command \"test\" not found!" > /dev/null
     CMDTESTEXIST=$?
 }
 [ ${TEST_COMMUNITY} = "YES" -a 0${CMDTESTEXIST} -eq 0 ] && {

   comtest="`GetCommandLine ${CSN}`"
   [ ${DEBUG} -gt 0 ] && echo "TEST_SEND: ${comtest}"
