...
CRC_Modbus()
{
  string=$(echo ${1} | sed 's/_0x//g;s/[_x#]//g')
  #echo $string >&2
  mode=${2}
  mode=${mode:=1}
  pref=${3}
  ...
  do
    ...
    # --- FOR SH END ----
     ...
     #echo ${string:$i:2} >&2
    #     crc=$(( crc ^ $((0x${string:$i:2})) ))   # ^ - по-битовое ИСКЛЮЧАЮЩЕЕ ИЛИ (XOR) FOR BASH
     crc=$(( crc ^ $((0x${bob})) ))   # ^ - по-битовое ИСКЛЮЧАЮЩЕЕ ИЛИ (XOR) FOR SH
     #for j in `seq 0 7`                      # & - по-битовое И (AND)
     for j in $masB
     do
         c=$(( crc & 1 ))
         crc=$(( crc >> 1 ))                  # >> - сдвиг вправо на 1 бит (деление на 2)
         [ $c -eq 1 ] && crc=$(( crc ^ 0xA001 ))
     done
...
ParserCommand()
{
  ...
  if [ 0"$3" -eq 0 ]; then
     ...
     if [ -n "`echo $nblock | grep "^[0-9][0-9]*$"`" ]; then
        cio="`echo -n $1 | awk -Fx -v num=$nblock -v stp=${START_BLOCK_DATA} '{print x$(num+stp)}'`"
     else
        cio=`echo $nblock | sed -n 's~^\(.*[dx]\)\(.*\)~echo "'$1'" | awk -Fx -v stp=${START_BLOCK_DATA} \\\{printf\\\(\\\"%\1\\\",\\\"0x\\\"\\\$\\\(\2+stp\\\)\\\)\\\}~p'`
     fi
  else
     # блочное значение
     cio="`echo "$nblock" | sed -n 's/\\\0/ /g; s/\(.*\)\\\t\(.*\)/\1'"$1"'\2/p'`"
     [ -n "$cio" ] && {
         echo "${cio}"
         return 0
     }
     ...
     if [ "`echo "${nblock}" | cut -c1`" = "A" ]; then
        nblock="`echo $nblock | sed 's~^A\(.*\)~{\1}~'`"
        #echo "1mnog=\"$mnog\" nblock=\"$nblock\" block=$block" >&2

        cio="`echo "${block}" | awk ${nblock}`"

        #echo "2mnog=\"$mnog\" nblock=\"$nblock\" block=$block" >&2
     else
        mnog="`echo $nblock | sed -n 's/.*\.\(.*\)f.*/\1/p'`"
        #echo "mnog=\"$mnog\" nblock=\"$nblock\" block=$block" >&2

        if [ -n "$mnog" ]; then
            cio=`echo $1 | awk '{printf("%'$nblock'", $0 / (10 ** '$mnog'))}'`
        else
            if [ "$nblock" = "$2" ]; then
                cio=`echo "$nblock" | sed -n 's/^a\(.*\)/\$(( '$1' \& 0x\1 ))/p'`
                #echo "c0=\"$mnog\" bio=\"$bio\" block=$block" >&2
                [ -z "$cio" ] && cio=`echo $nblock | printf "%$nblock" 0x${1}`
            else
               cio=$nblock
            fi
        fi
...
CheckParameter()
{
  ...
  par_cor="${par_def}"
  if [ -n "${par_val}" ]; then
       par_cor="`echo "${par_list}" | sed -n 's/^'${par_val}'	\(.*\)$/\1/p'`"
       #echo $par_cor >&2
       [ -z "${par_cor}" ] && {
           par_cor="$par_def"
           if [ -z "${par_flag}" ]; then
...
DoOutParser()
{
  ...
  for bl in `echo "${outpars}" | tr "|" " "`
  do
     ii=0
     #echo bl=$bl >&2
     for dv in `echo "$bl" | tr '#' " "`
     do
        #echo dv="$dv" >&2
        [ $dv = '0' ] && {
            echo -n "$block"
